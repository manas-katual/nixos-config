{ pkgs, config, lib, options, userSettings, ... }:
{
  config = lib.mkIf (config.my.desktop.option == "wayfire") {
    programs.wayfire = {
      enable = true;
...
max_render_time = -1
plugins = alpha   animate   autostart   command   cube   decoration   expo   fast-switcher   fisheye   foreign-toplevel   grid   gtk-shell   idle   invert   move   oswitch   place   resize   shortcuts-inhibit   switcher   vswitch   wayfire-shell   window-rules   wm-actions   wobbly   wrot   zoom
